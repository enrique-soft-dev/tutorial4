...
#         print("Corresponding Q-table cell to update:", position, action_column)
        ...
        if reward == 1 or reward == -1:
            self.q_table[pos][column] = (1 - self.alpha) * self.getQValue(state, action) + self.alpha * (reward + 0)
        else:
            best_action = self.getPolicy(nextState)
            self.q_table[pos][column] = (1 - self.alpha) * self.getQValue(state, action) + self.alpha * (reward + self.discount * self.getQValue(nextState, best_action))
